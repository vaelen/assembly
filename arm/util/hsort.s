...
hsort:  
    ...
    UDIV    R10, R12            // R10 = (len / 2) - 1 (starting heapify index)
    SUB     R10, R10, #1
...
heapify_check_left: 
    CMP     R4, R1              // Check to make sure the left node is within the array bounds
    BGE     heapify_check_right // If not then check the right node
...
heapify_check_right:    
    CMP     R5, R1              // Check to make sure the right node is within the array bounds
    BGE     heapify_swap        // If not then continue
    LDR     R8, [R0, R5, LSL#2] // R8 = Right value (R0 + (R5 * 4))
    CMP     R8, R7              // Compare right value to current largest value
    BLE     heapify_swap        // If left value <= largest value then continue
    MOV     R3, R4              // If left value > largest, change largest index to left node index
    MOV     R7, R8              //    and copy the value in preparation for right node comparison
heapify_swap:
    CMP     R2, R3              // Check to see if the largest is the initial index or not
    BEQ     heapify
    STRNE   R7, [R0, R2, LSL#2] // If not, store largest value at index (R0 + (R2 * 4))
    STRNE   R6, [R0, R3, LSL#2] //    and store index value at largest value index (R0 + (R3 * 4))
    MOVNE   R2, R3              //    and change index to largest index
    BNE     heapify             //    and recurse
heapify_next:
...
hsort_simple:
    ...
    STRLT   R3, [R0]            // Swap if out of order
    STRLT   R2, [R0, #4]
hsort_done:
